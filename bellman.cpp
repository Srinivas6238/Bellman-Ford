...
void aboutbtn1(int btn,int state,int x,int y)
{
if(btn==GLUT_RIGHT_BUTTON&&state==GLUT_DOWN)
Aboutflow();
//Aboutmethod();
...
void displayx (void)
{
	...
	drawstring(263, 410, "________________________________");
	glColor3f(1.5, 1.0, 1.0);
/*	drawstring(50, 375, "INSTRUCTIONS:");
	drawstring(50, 370, "____________");*/
	glColor3f(1, 0.5, 0.5);
	drawstring(60, 348, "Keyboard Functions");
	drawstring(60, 342, " ------------- ");
	glColor3f( 1.0,1.0,0.0);
   ...
   for(int r=1;r<=n;r++)
   {
             ...
             for(int x=0;x<l;x++)
             {
                     ...
                     if(p<q)
                     {
                            glColor3f(0.0,0.5,0.0);
                            glBegin(GL_LINES);
                                              glVertex2i(x1+500,y1);
                                              glVertex2i(x2+500,y2);
                            glEnd();

                            s1=itoa(cost[p][q],s,10);
                            drawstring((x1+x2+1000)/2,(y1+y2)/2,s1);
                     }
